...
const RenderPost = ({ post, redirect, preview }) => {
  ...
  // Generate description from post preview or title
  const postDescription = post.preview && post.preview.length > 0
    ? post.preview.map(block => block[0]).join(' ').substring(0, 160)
    : `Read ${post.Page} on Company Blog`
  ...
  const articleSchema = {
    ...
    author: post.Authors.length > 0 ? post.Authors.map(author => ({
      '@type': 'Person',
      name: author,
    })) : {
      '@type': 'Organization',
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${SITE_URL}${getBlogLink(post.Slug)}`,
    },
    url: `${SITE_URL}${getBlogLink(post.Slug)}`,
  }
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: post.Page,
        item: `${SITE_URL}${getBlogLink(post.Slug)}`,
      },
    ],
  }
  ...
  return (
    <>
      <Header titlePre={post.Page} description={postDescription} />
      <Head>
        {/* Article-specific meta tags */}
        <meta property="og:type" content="article" />
        <meta property="article:published_time" content={post.Date ? new Date(post.Date).toISOString() : ''} />
        {post.Authors.map((author, idx) => (
          <meta key={idx} property="article:author" content={author} />
        ))}
      ...
      <article className={blogStyles.post} itemScope itemType="https://schema.org/Article">
        <header>
          <h1 itemProp="headline">{post.Page || ''}</h1>
          {post.Authors.length > 0 && (
            <div className="authors" itemProp="author">By: {post.Authors.join(', ')}</div>
          )}
          {post.Date && (
            <div className="posted">
              Posted: <time dateTime={new Date(post.Date).toISOString()} itemProp="datePublished">
